...
function viewKaldik(arr)
{
	...
	setTimeout(function(){
		setTimeout(function(){
			...
			document.getElementById('tableNotaDinasContainer').style.display = 'none';
		...
		document.getElementById('tableNotaDinasContainer').className = 'pesanhilang';
	}, 300);
}
					...
					$.ajax({
						...
						success: function (data) {
							if(data[0].number != 0)
							{
								...
								while (i < data.length)
								{
									str += '<tr><td>'+j+'</td><td>'+data[i].pegawaiName+'</td><td><button type="button" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Ubah Data Pengajar" id="ubahpengajar;'+data[i].pengajarID+';'+data[i].pegawaiID+';'+data[i].pegawaiName+'" onclick="ubahPengajar(this.id);"><i class="fa fa-pencil"></i></button></td></tr>';
									i++;
									j++;
								}
...
function simpanDataPengajar()
{
	...
	{
		var uri = '<?php echo base_url(); ?>' + 'realisasijadwal/tambahpengajar/'+jadwalid+'/'+pegawai[0].pegawaiid+'/'+jmlhari;
...
function simpanNomorST()
{
	...
	var uri = '<?php echo base_url(); ?>' + 'realisasijadwal/simpannomorst/' + kelasid + '/' +nomorst+'/'+kaldikid;
		...
		setTimeout(function(){
			setTimeout(function(){
				setTimeout(function(){
					...
					document.getElementById('titleBoxND').innerHTML = 'Data Nota Dinas' + kaldikname + ' ' +  $("#kelas option:selected").html();
...
function showModal()
{
	...
	*/
	...
	uri = '<?php echo base_url(); ?>' + 'realisasijadwal/ambilsemuadatapegawai/';
				...
				success: function (data) {
					if(data[0].number != 0)
					{
						...
						while (i < data.length)
						{
							...
							var rowIndex = $('#example5').DataTable().row.add([data[i].pegawaiName]).draw();
...
function gantiJadwal(id)
{
	...
	//document.getElementById(iD).attribute("onclick","pilihPengganti();");
	document.getElementById(idButtonKlik).innerHTML = '<button type="button" class="btn btn-danger" id="'+id+'" onclick="resetGantiJadwal(this.id);"><i class="fa fa-close"></i></button>';
...
<!-- Main content -->
    <section class="content">
	  ...
	  <div class="row">
              ...
              <div class="box-tools pull-right">
				<button type="button" class="btn btn-primary btn-sm pull-right" data-widget="collapse" data-toggle="tooltip" title="Collapse" style="margin-right: 5px;">
				  ...
				  <button type="button" class="btn btn-primary btn-sm pull-right" style="margin-right: 5px;" onclick="buatND();"><i class="fa fa-envelope"></i> Buat ND</button>
				   <button type="button" class="btn btn-primary btn-sm pull-right" style="margin-right: 5px;" onclick="cetakND();"><i class="fa fa-envelope"></i> Cetak ND</button>
		...
		<div class="col-xs-12"></div>
		<div id="tableNotaDinasContainer" style="display:none;">
			<div class="box box-primary">
            <div class="box-header">
              <i class="ion ion-clipboard"></i>

              <h3 class="box-title" id="titleNotaDinas"></h3>
				
              <div class="box-tools pull-right">
				<button type="button" class="btn btn-primary btn-sm pull-right" data-widget="collapse" data-toggle="tooltip" title="Collapse" style="margin-right: 5px;">
                  <i class="fa fa-minus"></i></button>
				  <button type="button" class="btn btn-primary btn-sm pull-right" style="margin-right: 5px;" onclick="tambahND();"><i class="fa fa-plus"></i> tambah Nota Dinas</button>
            ...
            <div class="box-body">
				...
				<table id="example11" class="table table-bordered" cellspacing="0" style="table-layout: fixed;">
					...
					<tr>
					  ...
					  <td style="width:50px!important;">Perihal</td>
					  <td style="width:50px!important;">Pegawai</td>
					  <td style="width:50px!important;">Alasan</td>
		...
		<div id="formND" style="display:none;">
						...
						<div class="col-xs-12">
							...
							<div class="box-body">
									<label>Nomor ND</label>&nbsp&nbsp<i class="fa fa-star text-red"></i>
									<div class="form-group">
