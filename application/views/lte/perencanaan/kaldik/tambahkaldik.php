...
var idBefore = '';
var arrSplit, diklatid, diklatdesc, diklatlocid, diklatlocname, biayaid, biayaname, kodeasal, namaunitasal, kurikulumid, kurikulumname, kurikulumhari, hariLibur, totalHari, strHtml, kaldikMonth, kaldikYearIDm, l, str, idLast, strTable, ID, kurikulumjenisid, jenisname, totalHari, strSesi, metodeid, metodename, arrSesi;
strTable = [];
...
function pilihDiklatOk()
{
	...
	//var uri = '<?php echo base_url(); ?>' + 'kaldik/getkurikulum/?diklatid='+diklatid;
	/*
	var uri = '<?php echo base_url(); ?>' + 'kaldik/ambildetail/'+diklatid;
	$.ajax({
		type: 'get',
		url: uri,
		dataType: 'json',
		success: function (data) {
			...
			while (i < data.length)
			{
				str += '<option id="'+data[i].kurikulumID+';'+data[i].kurikulumName+'" value="'+data[i].kurikulumID+';'+data[i].kurikulumName+'">'+data[i].kurikulumName+'</option>';
				if(i == 0)
				{
					pilihKurikulum(data[i].kurikulumID+';'+data[i].kurikulumName);
				}
				i++;
			}
			//document.getElementById('listKurikulumJenis').innerHTML = str;
			document.getElementById('kurikulumName').innerHTML = str;
		}
	});
	*/
...
function pilihKurikulumOk()
{
	...
	if(totalHari > 0)
	{
		...
		var uri = '<?php echo base_url(); ?>' + 'kaldik/gethari/?kurikulumjenisid='+kurikulumjenisid;
		...
		$.ajax({
			...
			success: function (data) {
				...
				while (i < data.length)
				{
					...
					while (j < parseInt(data[i].maxhari))
					{
						if (temp.getDay() != 0)
						{	
							k = 0;
					...
					arrSesi.push({'sesiid':data[i].sesiID, 'startdate':tglMulai, 'enddate':tglSelesai, 'libur':totalLibur, 'minggu':minggu, 'hari':arrHari.length - totalLibur - minggu, 'sesidesc':data[i].sesiDesc, 'sesino':data[i].sesiNo,'peserta':1, 'jenisdesc':data[i].jenisDesc});
				...
				{
					strSesi += '<tr><td>'+arrSesi[i].jenisdesc+'</td><td>'+arrSesi[i].sesidesc+'</td><td>'+arrSesi[i].hari+'</td><td>'+arrSesi[i].libur+'</td><td>'+arrSesi[i].minggu+'</td><td>'+arrSesi[i].startdate+'</td><td>'+arrSesi[i].enddate+'</td><td>'+arrSesi[i].peserta+'</td><td><button type="button" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Ubah Data Sesi" id="ubah;'+arrSesi[i].sesiid+';'+arrSesi[i].startdate+';'+arrSesi[i].peserta+'" onclick="ubahSesi(this.id);"><i class="fa fa-pencil"></i></button></td></tr>';
	...
	{
		alert('Jenis Kurikulum yang anda pilih belum memiliki mata ajar');
	}
	*/
}
...
{
	var stat_blended, stat_elearning, stat_tatapmuka, rencanaAnggaran, kaldikDesc, kelas, KetUnitKerja, nost, tglst, uri, tglST, i, color, Kaldik, NonKaldik, kaldikStat, kurikulumid, metodeid, tempatname;
	var arrPertemuan = [];
	...
	{
		alert ('Anda Belum Mengisi Tempat DIKLAT Berlangsung');
	}
	else
	{
		...
		$.ajax({
			...
			success: function (data) 
			{
			  ...
			  if (data.success == 1)
			  {
					refreshPage('<?php echo base_url(); ?>kaldik?month=<?php echo $monthNumber; ?>&year=<?php echo $yearNumber; ?>');
			  }
			}
		});
    ...
    <section class="content">
      ...
      <h3>
		...
		<ol class="breadcrumb">
			...
			<li><a onclick="refreshPage('<?php echo base_url(); ?>kaldik');" style="cursor:pointer;"><i class="fa fa-calendar"></i> Kaldik</a></li>
			<li class="active" style="cursor:pointer;"><i class="fa fa-plus"></i> Tambah Kaldik</li>
		</ol>
      <!-- /.row -->
      <!-- Main row -->
      <div class="row">
		<div class="col-xs-12">
            ...
            <div class="box-body">
				<button type="button" class="btn btn-primary" data-widget="collapse" data-toggle="tooltip" title="Collapse" onclick="refreshPage('<?php echo base_url(); ?>kaldik?month=<?php echo $monthNumber; ?>&year=<?php echo $yearNumber; ?>')">
				...
				<div class="col-xs-12">
						...
						<div class="form-group">
                            <select class="form-control" id="metodeName" onchange="pilihMetode(this.value);">
							...
							<select id="biayaName" class="form-control" onchange="pilihBiaya(this.value)">
								...
								if($biayas)
								{
									...
									foreach($biayas as $biaya)
									{
										...
										{
											?>
											<script>pilihBiaya('<?php echo $biaya->biayaID;?>;<?php echo $biaya->biayaName;?>');</script>
											<?php
										}
										?>
						...
						<div class="col-xs-12">&nbsp</div>
						<div class="form-group">
							<button type="button" class="btn btn-primary pull-left" onclick="simpanKaldik();"><i class="fa fa-plus" ></i> Simpan KALDIK</button>
						</div>
				</div>
            </div>
